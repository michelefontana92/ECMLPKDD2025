...
@register_run('credit_alm_centralized_multiple')
class CreditALMCentralized(CreditRun):
    def __init__(self,**kwargs) -> None:
        super(CreditALMCentralized, self).__init__(**kwargs)
        ...
        self.num_groups = 4 if self.training_group_name == 'GenderAge' else 2
        ...
        for attributes in  self.sensitive_attributes:
            ...
            self.num_groups = 4 if group_name == 'GenderAge' else 2
        ...
        self.requirement_list = [ UnconstrainedRequirement(name='unconstrained_performance_requirement',
        ...
        for metric,group,threshold in zip(self.metrics_list,self.groups_list,self.threshold_list):
            print(f'Creating requirement for {metric}  on {group} with threshold {threshold}')
            self.threshold = threshold
            self.metric = metric
            self.training_group_name = group
            self.num_groups = 4 if self.training_group_name == 'GenderAge' else 2
            self.group_ids={self.training_group_name:list(range(self.num_groups))}
            constraint = SurrogateFactory.create(name=f'diff_{self.metric}',
                                                              surrogate_name=f'diff_{self.metric}_{group}',
                                                              weight=1,
                                                              average='weighted',
                                                              distributed_env=True,
                                                              group_name=self.training_group_name,
        ...
        self.requirement_set = RequirementSet(self.requirement_list)
    ...
    def setUp(self):
        ...
        self.metrics = [MetricsFactory().create_metric('performance')]
        for group_name in self.groups_list:
            self.num_groups = 4 if group_name == 'GenderAge' else 2
            self.group_ids={group_name:list(range(self.num_groups))}
            self.metrics += [
                        MetricsFactory().create_metric('demographic_parity',

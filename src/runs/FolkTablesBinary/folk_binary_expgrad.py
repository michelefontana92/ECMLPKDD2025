...
from wrappers import PytorchSklearnWrapper 
...
@register_run('folk_binary_expgrad')
class FolkTablesExpGradRun(FolkTablesBinaryRun):
    def __init__(self, **kwargs) -> None:
        super(FolkTablesExpGradRun, self).__init__(**kwargs)
    ...
    def run(self):
        ...
        if 'demographic_parity' in self.metrics_list:
            constraint = DemographicParity(difference_bound=0.20)
        else: 
            constraint = EqualizedOdds(difference_bound=0.20)

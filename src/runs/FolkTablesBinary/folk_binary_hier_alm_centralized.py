...
from wrappers import TorchNNHierarchicalLagrangianWrapper
...
@register_run('folk_binary_hier_alm')
class FolkTablesBinaryHierALMCentralized(FolkTablesBinaryRun):
    def __init__(self, **kwargs) -> None:
        ...
        self.batch_objective_function = SurrogateFactory.create(name='performance_batch', surrogate_name='cross_entropy', weight=1, average='weighted')
    ...
    def setUp(self):
        ...
        self.data_module = DataModule(dataset=f'{self.dataset}', root=f'data/{self.dataset.capitalize()}', train_set=f'{path}_train.csv', val_set=f'{path}_val.csv', test_set=f'{path}_val.csv', batch_size=self.batch_size, num_workers=0, sensitive_attributes=self.sensitive_attributes)
        ...
        self.wrapper = TorchNNHierarchicalLagrangianWrapper(
            model=self.model,
